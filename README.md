...
В папке `experiments/` лежат исследовательские Jupyter-ноутбуки
- `experiments/model_selection.ipynb` - выбор модели для индексации видео:
  при помощи оценки качества ранжирования (MAP, NDCG) на датасете `MultiVENT2.0`.

Ноутбук рассчитан на запуск в Google Colab с GPU
