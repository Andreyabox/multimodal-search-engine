# Система мультимодального анализа и поиска изображений и видео по пользовательским запросам.
...
- Осуществляет поиск видео (набор данных MultiVENT 2.0) по текстовому запросу: при помощи гибридной индексации используя отдельные именованные векторы `text` (caption + title) и `image` (усреднённый эмбеддинг кадров CLIP image-encoder).
